...
C_param_list_rd=C_param_list_cd
degree_list_rd=np.logspace(-3, 3, num=15)
...
X = validation_df_rd.columns.astype(float)  # C values
Y = validation_df_rd.index.astype(float)    # Degree values
X, Y = np.meshgrid(X, Y)
Z = validation_df_rd.astype(float).values
...
# Étiquettes
ax.set_title("Grid Search Validation polynomial kernel - F1-score (log scale)")
ax.set_xlabel('log10(C)')
ax.set_ylabel('log10(Degree of polynomial)')
ax.set_zlabel('F1-score')
...
X = training_df_rd.columns.astype(float)  # C values
Y = training_df_rd.index.astype(float)    # Degree values
X, Y = np.meshgrid(X, Y)
Z = training_df_rd.astype(float).values
...
# Étiquettes
ax.set_title("Grid Search Training polynomial kernel - F1-score (log scale)")
ax.set_xlabel('log10(C)')
ax.set_ylabel('log10(Gamma)')
ax.set_zlabel('F1-score')
...
print("Task 2 plotting finished — time to compute : {:.2f} secondes".format(time.time() - start))
...
# --- RBF kernel ---
# RBF kernel
nest_cv_rbf, rbf_grid = nested_10_fold_cv(C_param_list_cd, gam_list, 'rbf', train_sample, degr, 5)
print('\nUsing the RBF kernel we get a F1 average of: ', nest_cv_rbf)
plot_3d_surface(rbf_grid, title="Nested cross validation RBF Kernel - F1-score Grid Search")

#Nested cross validation for linear kernel
nest_cv_linear, linear_grid = nested_10_fold_cv(C_param_list_cd, gam_list, 'linear', train_sample, degr, 5)
print('\nUsing the linear kernel we get: ', nest_cv_linear.mean().mean())
#plot_3d_surface(linear_grid, title="Linear Kernel - F1-score Grid Search")
#define the grid for the degree
#degree_list=np.arange(2,10,1)
#degree_list=np.logspace(1, 5, 10)

##Nested cross validation for polynomial kernel
nest_cv_poly, poly_grid = nested_10_fold_cv(C_param_list_cd, gam_list, 'poly', train_sample, degree_list, 5)
print('\nUsing the polynomial kernel we get: ', nest_cv_poly.mean().mean())
#plot_3d_surface(poly_grid, title="Polynomial Kernel - F1-score Grid Search")
